...
You could wait for chips with matched cores. They are not coming. The reasons are architectural, and they are not going away. That leaves one conclusion, and it is uncomfortable for a single kernel: on this class of machine, the partition is not optional. Your only choice is whether it is a boundary you designed or a policy knob bolted on afterward.

## Two cores for two jobs

Silicon is not why the clusters disagree. Giving the general-purpose cores a 1024-bit register file would be a rounding error on the die, and [AMD's Zen 4](https://www.techinsights.com/blog/amd-zen-4-adds-avx-512) shows the cheap way to do it: full-width architectural registers over a narrower datapath. RISC-V vendors skip it because it buys them nothing. The width gap is a symptom of a decision made much earlier: the two clusters were built for opposite jobs.

An AI core is built for throughput per watt. Wide registers, many lanes, small caches, a lower clock. A general-purpose core is built for latency: deep out-of-order execution, large caches, a high clock, virtualization. On the K3 the AI cores clock at 2.0 GHz to the general cores' 2.4, carry half the L1, and lack the hypervisor extension. Vector width is a first-order knob on both designs, and each turned it where its job needed it. Force one width on both and one of them stops being good at its job.

Matching the width would not even bring migration back. The AI cores implement [SpacemiT's IME matrix instructions](https://github.com/spacemit-com/riscv-ime-extension-spec), the `vmadot` family, which the general cores do not, so a task that used them is pinned no matter what the vector length says. What matching the width would do is tax the wrong cluster. Every context switch on the general cores would save and restore 4 KB of vector state instead of 1 KB, and every signal frame would grow by 3 KB. The C library's `memcpy` uses vectors, so nearly every process is vector-dirty. The scalar cores that run most of the workload would pay on every switch, forever, for a migration nobody performs.

And nobody in RISC-V can force agreement anyway. [RVA23](https://github.com/riscv/riscv-profiles/blob/main/src/rva23-profile.adoc) mandates the vector extension, which sets a floor of 128 bits and no ceiling. The width is the implementation's choice. Core IP comes from different licensors, or from different teams inside one vendor, and each picks the width its target needs. ARM holds big.LITTLE clusters to one architectural state by contract. Intel, with both core types designed in-house, still spent two revisions of [AVX10](https://www.phoronix.com/news/Intel-AVX10) arguing with itself: first efficiency cores could stop at 256 bits, then the [2025 revision](https://www.phoronix.com/news/Intel-AVX10-Drops-256-Bit) made 512 mandatory everywhere. RISC-V has no contract and no one to make that call. Divergence is not a decision anyone made. It is the default.
...
Heterogeneous RISC-V servers are not a corner case on the roadmap. They are the roadmap, because the design pressures that produce them are not going away.
